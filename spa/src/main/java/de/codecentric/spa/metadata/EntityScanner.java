...
/**
 * Utility that scans entity class and returns {@link EntityMetaData} containing the information how should the given class be persisted.
 * 
...
public class EntityScanner {
    ...
    /**
     * Method scans the given class and puts it's descriptor in shape of {@link EntityMetaData} into {@link EntityMetaDataProvider}. Class hierarchy of the given class is scanned starting from the
     * given class to the most general class. Scanning process stops at the point where it comes to {@link Object} class.
     * 
     * If class is not annotated with {@link Entity} annotation, method will do nothing. Fields annotated with {@link Transient} annotation will be ignored since those should not be persisted.
     * 
     * Each class can have one identifier field (that field will be primary key in database table structure). Identifier field is field annotated with {@link Id} annotation. If there is more than one
     * field annotated as identifier, field that is first resolved as identifier will be kept.
     * 
     * @param cls
    ...
    /**
     * Method does actual scanning of the given class and calls itself recursively in order to retrieve information about all it's super classes.
     * 
     * @param cls
     *            class that will be scanned
        ...
        if (fields.length > 0) {
            for (Field f : fields) {
                if (!FieldScanner.isIdentifierField(f)) {
                    if (FieldScanner.isPersistentField(f)) {
     ...
     * 
     * Merging will be done only if right describing class {@link EntityMetaData} is super class of left describing class {@link EntityMetaData}.
     * 
     * @param left
     *            class being scanned
     * @param right
    ...
    public static class StringUtils {
         ...
         * 
         * Input string is converted from camel case to a string in a form of string which words are separated with "_". For example, for given input "SimpleClassName" table name will be
         * "simple_class_name".
         * 
         * @param input
