...
public class SQLGeneratorTest {
	...
	private static final String CREATE_SQL_CHILD = "CREATE TABLE dummy_attribute (id INTEGER PRIMARY KEY, dummy_name TEXT, dummy_value TEXT, dummy_attribute_parent_sub_entity_fk INTEGER, parent_id INTEGER, dummy_sub_entity_attributes_fk INTEGER, dummy_sub_entity_id INTEGER)";
	...
	private static final String INSERT_SQL_CHILD = "INSERT INTO dummy_attribute (dummy_name, dummy_value, dummy_attribute_parent_sub_entity_fk, dummy_sub_entity_attributes_fk, dummy_sub_entity_id, parent_id) VALUES (?, ?, ?, ?, ?, ?)";
	private static final String SELECT_ALL_SQL_CHILD = "SELECT id, dummy_name, dummy_value, dummy_attribute_parent_sub_entity_fk, dummy_sub_entity_id, dummy_sub_entity_attributes_fk, parent_id FROM dummy_attribute";
	private static final String SELECT_SINGLE_SQL_CHILD = "SELECT id, dummy_name, dummy_value, dummy_attribute_parent_sub_entity_fk, dummy_sub_entity_id, dummy_sub_entity_attributes_fk, parent_id FROM dummy_attribute WHERE id = ?";
	private static final String UPDATE_SQL_CHILD = "UPDATE dummy_attribute SET dummy_name = ?, dummy_value = ?, dummy_attribute_parent_sub_entity_fk = ?, dummy_sub_entity_id = ?, dummy_sub_entity_attributes_fk = ?, parent_id = ? WHERE id = ?";
	...
	public void setUp() {
		...
		childMetaData = entityMetaDataProvider.getMetaData(DummyAttribute.class);
	}
	...
	public void testGenerateSQL() {
		...
		Assert.assertTrue(checkCreateSQLStringContents(CREATE_SQL_CHILD, childSQL.getCreateTableSQL()));
		...
		Assert.assertTrue(checkCreateSQLStringContents(INSERT_SQL_CHILD, childSQL.getInsertSQL()));
		Assert.assertTrue(checkSelectSQLStringContents(SELECT_ALL_SQL_CHILD, childSQL.getSelectAllSQL()));
		Assert.assertTrue(checkSelectSQLStringContents(SELECT_SINGLE_SQL_CHILD, childSQL.getSelectSingleSQL()));
		Assert.assertTrue(checkUpdateSQLStringContents(UPDATE_SQL_CHILD, childSQL.getUpdateSQL()));
	}
	...
	private static boolean checkUpdateSQLStringContents(String expected, String actual) {
		boolean result = expected.substring(expected.indexOf("WHERE"))
				.equals(actual.substring(actual.indexOf("WHERE")));
